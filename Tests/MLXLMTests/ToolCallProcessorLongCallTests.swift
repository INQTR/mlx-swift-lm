...
/// `ToolCallProcessor` while collecting a tagged call finds the end tag by
/// scanning the chunk plus the tag's overlap with the buffered text, not the
/// whole buffered call: a long call streamed token by token costs time
/// linear in its length, and the tag is still found wherever a chunk
/// boundary falls.
