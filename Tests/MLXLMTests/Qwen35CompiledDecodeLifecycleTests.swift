...
//
// Coverage spans all three generations of closures: the plain-cache decode
// installs the C14 per-layer traces (first token, no GDN state yet) and the
// whole-step segment traces (second token); the quantized-cache decode
// forces the fallback that installs the MoE block's own C11 closure — the
// one the original regression was filed against.
...
final class Qwen35CompiledDecodeLifecycleTests: XCTestCase {
    ...
    func testBlocksDeallocateAfterQuantizedCacheDecode() throws {
        // A quantized KV cache makes the C14 whole-step schedule and the
        ...
        // (C11). GDN layers still take the per-layer trace.
        ...
        XCTAssertNil(
            ...
            "Qwen35SparseMoeBlock leaked after quantized-cache decode — the "
                + "C11 compiled closure must not retain its module")
    }
}
