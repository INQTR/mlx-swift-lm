...
/// Runtime switch over ``LLMModel/prepare(_:cache:windowSize:)``'s chunking strategy.
///
...
extension LLMModel {
    ...
    /// GEMM. Measured on Qwen3.6-35B-A3B at 32K (tesseract #253/#258), a 141-token
        ...
        -> PrepareResult
    {
        ...
        // chunk N.
        //
        // Chunks are *balanced*: `prefillStepSize` is a ceiling, and the prompt is
        // divided into the fewest chunks that respect it, each of equal size. The
        // naive `while size > prefillStepSize` loop instead leaves a remainder of
        // `promptTokens mod prefillStepSize`, and that degenerate forward is
        // disproportionately slow — it runs at the largest `Lk` of the whole
        // prefill while giving each MoE expert only `Lq × topK / numExperts` rows
        // (four rows, for a 141-token remainder on a 256-expert model). Measured on
        // Qwen3.6-35B-A3B at 32K: a 141-token remainder cost 2.89 s, against
        // ~0.99 s for a full 1024-token chunk.
        //
        // Balancing removes the remainder without adding a forward: 31,884 tokens
        // at a 1024 ceiling become 32 chunks of 997 rather than 31 of 1024 plus a
        // straggler of 140.
        //
        // A prompt that fits in one chunk is handed to the iterator whole, exactly as
        // before: chunking it would only add a second forward.
        let chunkSize: Int
        let tail: Int
