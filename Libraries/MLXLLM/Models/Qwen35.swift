...
final class Qwen35Attention: Module {
    ...
    /// compiled region, so the arithmetic is untouched (C14).
...
final class Qwen35SparseMoeBlock: Module, UnaryLayer {
    ...
    /// (C14 milestone B) inlines it directly rather than nesting this
    /// module's own compiled wrapper inside that trace.
    ...
    /// C18 (tesseract): pick the top-`k` experts and normalise their scores.
    ///
...
final class Qwen35DecoderLayer: Module {
    ...
    func callAsFunction(
        ...
        // C14 (tesseract), milestone B: the whole decode-step layer — norm →
        // attention → residual → norm → MLP → residual — runs as one traced
        // function per layer instance (two for full-attention layers, split
        // where the KV cache is written). It generalises C11/C12, which
        // compiled the MoE and GDN bodies but left the norms, residuals and
        // the whole full-attention layer to be rebuilt op-by-op from Swift
        // every token. Fusion stays in the E2-proven bitwise class; the parity
        // gate arbitrates. Prefill, masked/array-mask calls and cache kinds
        // with their own attention route (quantized, turbo) keep the path
        // below unchanged.
    ...
    // MARK: - Compiled decode blocks (C14)
    ...
    /// function has to be pure — exactly as in C12; the first decode token
    /// supplies the same explicit zero states `gatedDeltaUpdate` would have
    /// built internally.
    private func decodeLinearLayer(_ x: MLXArray, cache: MambaCache) -> MLXArray {
        ...
        if compiledLinearLayer == nil {
            ...
            // release (the C11/C12 leak). The trace also bakes the weights it
...
public class Qwen35TextModelInner: Module {
    ...
    func callAsFunction(_ inputs: MLXArray, cache: [KVCache?]? = nil) -> MLXArray {
        // C14 (tesseract), milestone C: the whole decode step runs as a small
    ...
    // MARK: - Whole-step decode schedule (C14 milestone C)
    ...
    private func decodeStep(_ inputs: MLXArray, _ cache: [KVCache?]) -> MLXArray? {
        ...
        for (segmentIndex, segment) in decodeSegments.enumerated() {
            ...
            if compiledSegments[segmentIndex] == nil {
                ...
                // tapes on every release (the C11/C12 leak). The traces bake
